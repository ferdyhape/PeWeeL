...
<body>
    <div class="container mt-5 mb-5 d-flex justify-content-center">
        ...
        <div class="col-lg-5 shadow p-5 bg-light" id="form-all">
            <h2 class="h3 text-center mb-4">Edit Data</h2>
            <form action="/home/{{ $product->id }}" method="POST">
                @method('PUT')
                @csrf
                <div class="mb-3">
                    <label for="product_name" class="form-label">Product Name</label>
                    <input type="text" class="form-control @error('product_name') is-invalid @enderror" id="product_name" name="product_name" required autofocus value="{{ $product->product_name }}">
                    @error('product_name')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('category') is-invalid @enderror" id="category" name="category" required autofocus value="{{ $product->category }}">
                    @error('category')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
